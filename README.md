...
This repository now ships the Odoo snippet collection originally created for VS Code by the [Droggol/VscOdooSnippets](https://github.com/Droggol/VscOdooSnippets) project as four separate Zed extensions:
...
## Installation (Zed)
1. Copy or symlink the desired language folder (for example `odoo-python-snippets/`) into your Zed extensions directory (typically `~/.config/zed/extensions/`).
